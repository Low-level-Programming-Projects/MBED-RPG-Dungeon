...
{
    //****************
    unsigned int hash = key%MHF_NBUCKETS;
    return hash;
    //****************
...
{
    //****************
    int area = maps[active_map].w * maps[active_map].h;
    return area;
    //****************
...
MapItem* get_north(int x, int y)
{
    //***************
    // load the hashmap
    // get item with key north of coordinate after you get the hash of the coordinates
    int xN = x;
    int yN = y + 1;
    int key = XY_KEY(xN,yN);
...
void add_big_plant(int x, int y)
{
    MapItem* w1 = (MapItem*) malloc(sizeof(MapItem));
    w1->type = BIGPLANT;
    w1->draw = draw_big_plant;
    w1->walkable = true;
    w1->data = NULL;
    void* val = insertItem(get_active_map()->items, XY_KEY(x, y), w1);
    if (val) free(val); // If something is already there, free it
...
void add_talking_npc(int x, int y)
{
    //****************
    MapItem* w1 = (MapItem*) malloc(sizeof(MapItem));
    w1->type = NPC;
    w1->draw = draw_talking_npc;
    w1->walkable = false;
    w1->data = NULL;
    void* val = insertItem(get_active_map()->items, XY_KEY(x, y), w1);
    if (val) free(val); // If something is already there, free it
...
void add_apple_tree(int x, int y)
{
    //****************
    MapItem* w1 = (MapItem*) malloc(sizeof(MapItem));
    w1->type = APPLETREE;
    w1->draw = draw_apple_tree;
    w1->walkable = false;
    w1->data = NULL;
    void* val = insertItem(get_active_map()->items, XY_KEY(x, y), w1);
    if (val) free(val); // If something is already there, free it
...
void add_empty_tree(int x, int y)
{
    //****************
    MapItem* w1 = (MapItem*) malloc(sizeof(MapItem));
    w1->type = EMPTYTREE;
    w1->draw = draw_empty_tree;
    w1->walkable = false;
    w1->data = NULL;
    void* val = insertItem(get_active_map()->items, XY_KEY(x, y), w1);
    if (val) free(val); // If something is already there, free it
...
void add_dead_skull(int x, int y)
{
    //****************
    MapItem* w1 = (MapItem*) malloc(sizeof(MapItem));
    w1->type = DEADSKULL;
    w1->draw = draw_dead_skull;
    w1->walkable = true;
    w1->data = NULL;
    void* val = insertItem(get_active_map()->items, XY_KEY(x, y), w1);
    if (val) free(val); // If something is already there, free it
